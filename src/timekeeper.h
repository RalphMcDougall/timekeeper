...
namespace TimeKeeper
{
    ...
    class Tracker
    {
        ...
        tkll flag;                      // The value that is used to track execution time versus some variable

        public:
        Tracker(const std::string, const bool = false, const tkll = 0);     // Standard constructor where just the tracker name is passed
        Tracker(const std::string, const std::string, const std::string, const bool = false, const tkll = 0);   // Special constructor where the project name and program name are given too so that the TimeKeeper registering can be performed in the same step, simplifying the library usage
