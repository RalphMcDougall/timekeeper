...
namespace TimeKeeper
{
    ...
    void addEvent(const EventType event_type, const std::string event_issuer)
    {
    ...
    Tracker::Tracker(const std::string _tracker_name, const bool _project_events)
    {
        ...
        addEvent(project_events ? PROJECT_TRACKING_START : TRACKING_START, tracker_name);
    }
    ...
    {
        if (recorded) return;
        addEvent(project_events ? PROJECT_TRACKING_END : TRACKING_END, tracker_name);
        recorded = true;
    }
}